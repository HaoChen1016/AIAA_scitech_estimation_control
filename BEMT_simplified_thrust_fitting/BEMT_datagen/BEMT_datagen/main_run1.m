...
RPM_min = 8000;RPM_max = 16000;RPM_icrm = 1000;
    ...
    for j = Vr_y_min:Vr_y_icrm:Vr_y_max
        ...
        for k = Vr_z_min:Vr_z_icrm:Vr_z_max
            ...
            for l=RPM_min:RPM_icrm:RPM_max
                ...
                [T_final,lambda1,lambda2]  = FWF_BEMT_adjustment(RPM_now,V_rel_b, P);
...
for m=1:1:test_data_len
    inp_v = [randi([-5,5]), randi([-5,5]), randi([-5 5])];
    while norm(inp_v) ==0
        inp_v = [randi([-5,5]), randi([-5,5]), randi([-5 5])];
    end
    
    inp_rpm = ones(4,1)*randi([8000 16000]);
    
    [inp_v inp_rpm']
    [T_final,lambda1,lambda2]  = FWF_BEMT_adjustment(inp_rpm,inp_v, P);
